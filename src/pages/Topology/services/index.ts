...
import { NODE_SIZE } from '@core/components/Graph/config';
...
export const TopologyController = {
  ...
  convertSitesToGroups: (processes: ProcessResponse[]): NodeModel[] => {
    const comboMap = processes.reduce(
      (acc, node) => {
  ...
  addMetricsToLinks: (
    ...
    return links.map((link: any) => {
      ...
      const byterateReverse = byteRateByProcessPairsMap[`${link.sourceName}${link.targetName}`];
